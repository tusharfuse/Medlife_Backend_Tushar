...
# ---- Twilio / SMS Config ----
TWILIO_ACCOUNT_SID = os.getenv("TWILIO_ACCOUNT_SID", "AC81d03ae32dab17e32324af12b3f0ccc3")
TWILIO_AUTH_TOKEN = os.getenv("TWILIO_AUTH_TOKEN", "d57f73ae878eebed40577b85c8640625")
...
def send_email_otp(email: str, otp: str) -> bool:
    ...
    Send OTP via email using your PrivateEmail SMTP settings:
      ...
      - SSL on 465 (no STARTTLS)
    Falls back to dev print if credentials are missing.
    """
    try:
        print("host use:",EMAIL_HOST_USER)
        # Ensure we actually have credentials
        if not EMAIL_HOST_USER or not EMAIL_HOST_PASSWORD:
            logging.warning("SMTP credentials missing; printing OTP (dev mode)")
            print(f"[DEV EMAIL] To: {email} | OTP: {otp}")
            return True
        ...
        msg["From"] = EMAIL_FROM
        msg["To"] = email
        msg["Subject"] = "Your MedLife OTP Code - Secure Login"
...
"""
        ...
        # Connect & send
        if EMAIL_USE_SSL:
            # SSL connection on 465
            server = smtplib.SMTP_SSL(EMAIL_HOST, EMAIL_PORT, timeout=30)
            server.login(EMAIL_HOST_USER, EMAIL_HOST_PASSWORD)
            server.send_message(msg)
            server.quit()
        else:
            # Plain or STARTTLS (not used in your config, but kept for completeness)
        ...
        logging.error(f"Failed to send email OTP: {e}")
        return False
def to_e164(number: str) -> str:
    """Best-effort convert a user-entered number to E.164.
    If it already starts with '+', keep it; otherwise prefix default country code.
    ...
    if cleaned.startswith("+"):
        return cleaned
    # assume local dialing without country code
...
def send_sms_otp(mobile: str, otp: str) -> bool:
    ...
    - Uses Twilio if env vars are present.
    - Falls back to console print in dev if not configured.
    """
    try:
        # If Twilio not configured, dev fallback
        ...
        msg = client.messages.create(
            body=body,
            from_=to_e164(TWILIO_FROM_NUMBER),
            to=to_number
        )
